...
facilitates the testing of the widget class in a way the mimics its actual runtime usage.  What the harness does is render
...
of events, setting properties, and children of the widget class and observing how that changes the virtual DOM render and how
...
special virtual DOM nodes before being sent to the virtual DOM engine for rendering.  They are custom element tags which will look
...
were no errors, or rejects with the first error collected.  For example:
...
Third, `DNode`s will not be rendered during comparison.  Simply their children will be walked, but if a `DNode`'s rendering causes
