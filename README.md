...
|`ignoreProperties`| |When comparing, properties that should just be ignored, where their presence is not considered in either the actual or expected render.  This can be an array of strings or regular expression, or it can be a callback function which takes the arguments of `property, a, b` where `property` is the string name of the propery being compared and `a` is the object from the `actual` and `b` is the object from the expected being compared.  The function should return `true` if the property should be ignored or `false` if it should be compared.|
|`ignorePropertyValues`|`[ 'bind' ]`|When comparing, properties that only the presence of the property should be evaluated and the value ignored.  This can be an array of strings or regular expression, or it can be a callback function which takes the arguments of `property, a, b` where `property` is the string name of the propery being compared and `a` is the object from the `actual` and `b` is the object from the expected being compared.  The function should return `true` if the property value should be ignored or `false` if it should be compared.|
